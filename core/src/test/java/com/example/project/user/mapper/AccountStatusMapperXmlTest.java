...
class AccountStatusMapperXmlTest {
    ...
    @Test
    @DisplayName("관리자 목록용 만료 차단 해제 SQL은 만료된 BLOCKED 회원만 갱신한다")
    void activateAllExpiredBlocksWithNarrowCondition() throws Exception {
        Configuration configuration = configuration();
        String statement = "com.example.project.user.mapper.AccountStatusMapper.activateAllExpiredBlocks";
        String sql = configuration.getMappedStatement(statement)
                .getBoundSql(Map.of())
                .getSql()
                .replaceAll("\\s+", " ")
