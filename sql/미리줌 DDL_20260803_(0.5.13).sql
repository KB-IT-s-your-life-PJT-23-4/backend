CREATE DATABASE IF NOT EXISTS `mirizoom`
...
CREATE TABLE `kb_product_version` (
                                      ...
                                      INDEX `idx_kb_product_version_data_version_sales_status`
                                          (`kb_product_data_version_id`, `sales_status`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `etf` (
                       ...
                       `annual_return_5y` DECIMAL(8,4) NOT NULL
                           COMMENT '5년 연평균 수익률(%), 음수 허용',
                       `bond_ratio` DECIMAL(7,4) NOT NULL
                           COMMENT '채권 비중(%)',
                       `risk_level` ENUM(
                           'EX_LOW',
                           'LOW',
...
CREATE TABLE `etf_holding` (
                               ...
                               INDEX `idx_etf_holding_version_rank`
                                   (`kb_product_version_id`, `holding_rank`),

                               INDEX `idx_etf_holding_name`
                                   (`holding_name`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `base_interest_rate` (
                                      ...
                                      INDEX `idx_base_interest_rate_version_month`
                                          (`kb_product_version_id`, `min_month`, `max_month`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `preferential_interest_rate` (
                                              ...
                                              INDEX `idx_preferential_interest_rate_version`
                                                  (`kb_product_version_id`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `simulation` (
                              ...
                              INDEX `idx_simulation_family_history`
                                  (`family_id`, `updated_at` DESC, `simul_id` DESC),

                              INDEX `idx_simulation_family_status`
                                  (`family_id`, `status`),

                              INDEX `idx_simulation_status_expiry`
                                  (`status`, `expired_at`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `simulation_product` (
                                      ...
                                      INDEX `idx_simulation_product_portfolio_selected`
                                          (`simul_portfolio_id`, `is_selected`)
) ENGINE=InnoDB
  DEFAULT CHARSET=utf8mb4
  COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `gift_deduction_limit` (
                                        ...
                                        `effective_from` DATE NOT NULL
                                            COMMENT '이 공제 한도가 적용되기 시작하는 증여일(법 시행일)',
...
SELECT
    ...
    CASE
        WHEN COALESCE(g.completed_count, 0) > 0 THEN 1
        ELSE 0
        END AS has_previous_gifts,
    g.previous_gift_amount,
    g.previous_gift_date,
    CASE
        WHEN g.oldest_gift_date IS NULL THEN NULL
        WHEN DATE_ADD(g.oldest_gift_date, INTERVAL 10 YEAR) <= CURRENT_DATE
            THEN DATE_ADD(g.oldest_gift_date, INTERVAL 20 YEAR)
        ELSE DATE_ADD(g.oldest_gift_date, INTERVAL 10 YEAR)
...
FROM family f
    ...
    SELECT
        ...
        MIN(gift_date) AS oldest_gift_date
    FROM gift
    WHERE status = 'COMPLETED'
      AND gift_date >= DATE_SUB(CURRENT_DATE, INTERVAL 10 YEAR)
      AND gift_date < CURRENT_DATE
    GROUP BY family_id
) g
